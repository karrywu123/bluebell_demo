...
func Test_SignUpHandler(c *gin.Context){
	var p models.RegisterForm
	fmt.Println(p)
	fmt.Println(p.UserName)
	if err := c.ShouldBindJSON(&p); err !=nil{
		zap.L().Error("sigup whith invalid param ",zap.String("xxx","www"),zap.Error(err))
...
func SignUpHandler(c *gin.Context)  {
	...
	if err := c.ShouldBindJSON(&fo); err != nil {
		zap.L().Error(" 用户密码填写错误",zap.String("注册用户",fo.UserName),zap.Error(err))
