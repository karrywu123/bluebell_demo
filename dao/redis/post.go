...
func PostVote(postID, userID string, v float64) (err error) {
	...
	ov := client.ZScore(ctx, key, userID).Val() // 获取当前分数

	diffAbs := math.Abs(ov - v)
	pipeline := client.TxPipeline()

	pipeline.ZAdd(ctx,key,&redis.Z{Score:v,Member:userID})//记录已经投票的
	pipeline.ZIncrBy(ctx,KeyPostScoreZSet, VoteScore*diffAbs*v, postID) // 更新分数
...
func GetCommunityPost(communityName, orderKey string, page int64) []map[string]string {
	...
	if client.Exists(ctx,key).Val() < 1 {
		//client.ZInterStore(key, redis.ZStore{Aggregate: "MAX",}, KeyCommunityPostSetPrefix+communityName, orderKey)
		client.ZInterStore(ctx,key,&redis.ZStore{Aggregate:"max"})
